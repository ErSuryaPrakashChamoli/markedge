...
@endphp
{{-- Compact by design: the hero and the section after it both fit a desktop viewport without scrolling. --}}
<x-ui.section :theme="$data['theme'] ?? 'dark'" pattern="mesh" spacing="sm" :id="$data['anchor'] ?? null" as="div" class="flex flex-col justify-center !py-8 lg:!py-10">
    <div @class([
        'grid grid-cols-1 gap-8 lg:items-center',
        ...
        <div @class(['max-w-3xl py-2 lg:py-4', 'mx-auto' => $center])>
            @if (filled($data['eyebrow'] ?? null))
                <x-ui.eyebrow>{{ $data['eyebrow'] }}</x-ui.eyebrow>
            @endif
            <h1 class="mt-5 text-h1">{{ $lead }}@if ($highlightCount > 0) <span class="text-gradient-brand">{{ $highlight }}</span>@endif</h1>
            @if (filled($data['subheading'] ?? null))
                <p class="mt-4 max-w-2xl text-body-lg text-fg-secondary {{ $center ? 'mx-auto' : '' }}">{{ $data['subheading'] }}</p>
            @endif
            @if ($data['primaryCta'] || $data['secondaryCta'])
                <div class="mt-6 flex flex-wrap gap-4 {{ $center ? 'justify-center' : '' }}">
                    @if ($data['primaryCta'])
